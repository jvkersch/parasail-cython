...
PROFILE_SIGNATURE = """\
    parasail_profile_t* _c_{name} "{name}"(
        const char * const s1, const int s1Len,
        const parasail_matrix* matrix)
"""
...
def write_parasail_profile_defs(fname):
    instruction_set = ['_sse_128', '_avx_256', '_altivec_128']
    width = ['_8', '_16', '_32', '_64', '_sat']
    stats = ['', '_stats']
    with open(fname, "a") as fp:
        print(PROFILE_HEADER, file=fp)
        for s, i_s, w in product(stats, instruction_set, width):
            name = "parasail_profile_create{}{}{}".format(s, i_s, w)
            print(PROFILE_SIGNATURE.format(name=name), file=fp)
...
def main():
    ...
    write_parasail_profile_defs(ns.output)
